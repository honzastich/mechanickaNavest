...
#define bit3 A0                   // přímo / odbočka / treti rameno
...
byte _servo3_pos = 0;    // startovni pozice serva 3

byte _servo_limtPos11 = 0;  // poloha serva 1 - VOLNO   1
byte _servo_limtPos12 = 0;  // poloha serva 1 - STUJ    2
byte _servo_limtPos21 = 0;  // poloha serva 2 - VOLNO   3
byte _servo_limtPos22 = 0;  // poloha serva 2 - STUJ    4
byte _servo_limtPos31 = 0;  // poloha serva 3 - VOLNO   5
byte _servo_limtPos32 = 0;  // poloha serva 3 - STUJ    6
...
Flasher f_ledUp(ledUp, 200);  // 2 rameno
Flasher f_ledDown(ledDown, 200);

Flasher f_ledUp3(ledUp, 100);  //3 rameno
Flasher f_ledDown3(ledDown, 100);
...
void servoSetPosition(bool prgStart, byte servoStatus) {
  // nastavuje obe serva podle stausu (0 - STUJ, 1 - VOLNO , 3 - VOLNO 40, 9 ocekqavej 40)
  ...
  byte aktPos1 = _servo1.read();
  byte aktPos2 = _servo2.read();
  byte aktPos3 = _servo3.read();
  if (prgStart == true) {
    aktPos1 = (_servo_limtPos11 + _servo_limtPos12) / 2;
    aktPos2 = (_servo_limtPos21 + _servo_limtPos22) / 2;
    aktPos3 = (_servo_limtPos31 + _servo_limtPos32) / 2;
  }
  ...
  if (runMove == true) {
    if (aktPos1 < endPos1) {
      ...
      for (i1 = aktPos1; i1 <= endPos1; i1 += 1) {
        ...
        i3 = myMap(i1, aktPos1, endPos1, aktPos3, endPos3);
        _servo1.write(i1);
        _servo2.write(i2);
        _servo3.write(i3);
        delay(setServoPosDelay);
...
void navestidloIni(bool prgStart) {
  ...
  // servo 3
  _servo_limtPos31 = EEPROM.read(5);  // volno
  _servo_limtPos32 = EEPROM.read(6);  // stuj
  /* inicializace, kdyz v EEPROM jeste nic neni*/
  if (_servo_limtPos31 == _servo_limtPos32 && _servo_limtPos31 > 180) {
    _servo_limtPos31 = 93;
    _servo_limtPos32 = 93;
  }
  ...
  */
  ...
  switch (value) {
    ...
    case 9:  // volno do odbocky
      _servo1_pos = 1;
      _servo2_pos = 1;
      _servo3_pos = 1;
      servoSetPosition(prgStart, 9);
      break;
  }
}
...
long myMap(long x, long in_min, long in_max, long out_min, long out_max) {
  ...
  if (abs(in_size) > abs(out_size)) {
    if (in_size < 0) in_size--;
    else in_size++;
    if (out_size < 0) out_size--;
    else out_size++;
  }
    ...
    if ((millis() - lastDebounceTime) >= bitDebounceTime)  //pockam 0.5 sed od zmeny stavu
    {
      if (bit1State != _bit0) {
        ...
        if (_bit0 == 1) {
          ...
          _bit3 = !digitalRead(bit3);  // nacitam jen pri bit0=1
        }

        if (_bit0 == 0)
          bitValue = 0;

        if (_bit0 == 1 && _bit1 == 0 && _bit3 == 0)
          bitValue = 1;

        if (_bit0 == 1 && _bit1 == 1 && _bit3 == 0)
          bitValue = 3;
...
void prgMode() {
  ...
  switch (modePrgStep) {
    ...
    case 5:  // 3-ti rameno - volno
      digitalWrite(ledDown, LOW);
      f_ledUp3.LEDflash();
      break;

    case 6:  //dolni navest - stuj
      digitalWrite(ledUp, LOW);
      f_ledDown3.LEDflash();
      break;
  }
    ...
    if (modePrg == false) {
      ...
      if (modePrgStep < 7) {
        switch (modePrgStep) {
          ...
          case 5:
            _servo_limtPos22 = _servo_aktPos;                    // nahraju posledni pozici serva 2
            servoSetDefaultPosition(_servo2, _servo_limtPos21);  // servo 1 dam do polohy volno - je to kvuli programovani tretiho ramene

            _servo2.detach();

            // prepnu na treti servo
            _servo3.attach(servo3);
            servoSetDefaultPosition(_servo3, _servo_limtPos31);  // servo 2 dam do polohy VOLNO
            _servo_aktPos = _servo3.read();
            break;

          case 6:
            _servo_limtPos31 = _servo_aktPos;                    //
            servoSetDefaultPosition(_servo3, _servo_limtPos32);  // servo j dam do polohy STUJ
            _servo_aktPos = _servo3.read();
            break;
        }
        //Serial.println("Krok: " + String(modePrgStep, DEC) + " pozice serva: " + String(_servo_aktPos, DEC));
      } else {
        _servo_limtPos32 = _servo_aktPos;  //  ulozim si polohu STUJ pro 2. servo
        _servo3.detach();
        modePrg = false;
        ...
        EEPROM.update(5, _servo_limtPos31);
        EEPROM.update(6, _servo_limtPos32);
        /*
